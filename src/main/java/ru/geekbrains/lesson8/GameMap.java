 ...
 * v1.0
 */
public class GameMap extends JPanel {                      // объявляется класс GameMap явлюящися расширением JPanel
...
//    private static int scoreAI = 0;
        ...
        for (int y = 0; y < fieldSizeY; y++) {
            for (int x = 0; x < fieldSizeX; x++) {
                ...
                if (field[y][x] == DOT_HUMAN) {
                    ...
                    g.fillOval(x * cellWidth + DOT_PADDING + 1,
                            y * cellHeight + DOT_PADDING+1,
                            cellWidth - DOT_PADDING * 2,
                            cellHeight - DOT_PADDING * 2);
                    g.setColor(new Color(235, 235, 235));
                    g.fillOval(x * cellWidth + DOT_PADDING *2+1,
                            y * cellHeight + DOT_PADDING *2+1,
                            cellWidth - DOT_PADDING *4 ,
                            cellHeight - DOT_PADDING * 4);

                } else if (field[y][x] == DOT_AI) {
                    g.setColor(Color.RED);
                    g.fillRect ((int) (x * cellWidth + cellWidth * 0.45),
                             y * cellHeight + DOT_PADDING,
                            (int) (cellWidth * 0.15),
                            cellHeight - DOT_PADDING *2);
                    g.fillRect((int) (x * cellWidth + DOT_PADDING ),
                            (int) (y * cellHeight + cellHeight * 0.45),
                            cellWidth - DOT_PADDING*2,
                            (int) (cellHeight * 0.15));
                } else {
    ...
    private void showMessageGameOver(Graphics g) {
        ...
        switch (stateGameOver) {
            case STATE_DRAW:
                int res = (int)(getFont().getSize() * 2.9);
                g.drawString("DRAW", 135, getHeight() / 2 + res);
                break;
            case STATE_WIN_HUMAN:
                res = (int)(getFont().getSize() * 2.9);
                g.drawString("HUMAN Wins!", 90, getHeight()/ 2 + res);
                break;
            case STATE_WIN_AI:
                res = (int)(getFont().getSize() * 2.9);
                g.drawString("AI Wins!", 170, getHeight()/ 2 + res);
                break;
        }
    }
